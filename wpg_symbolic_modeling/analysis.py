...
def build_wpg_for_library(file_path, analyze=True, build=True, activate= False, general=True):
	...
	@param {bool} build
	@param {bool} activate
	@param {bool} general: specifies whether to save in the general libraries folder or is site specific
	@return {void} None
	"""
	...
	def _analyze():
		...
		relative_output_directory = utilityModule.remove_part_from_str(file_path, remove_str)
		relative_output_directory = utilityModule.get_directory_without_last_part(relative_output_directory)

		command = "node %s -js %s -o %s"%(constantsModule.ANALYZER_DRIVER_PATH, file_path, relative_output_directory)
	...
	def _prepare_db():
		...
		neo4j_csv_absolute_path = utilityModule.get_directory_without_last_part(file_path)
		neo4jDatabaseUtilityModule.API_neo4j_prepare(neo4j_csv_absolute_path)
		time.sleep(3)
	...
	def _activate_db():
		...
		db_name = library_name + '.db'
		if neo4jDatabaseUtilityModule.does_neo4j_db_exists(db_name):

			neo4jDatabaseUtilityModule.activate_existing_neo4j_db(db_name)
...
def query_library_database_for_signatures(tx, library_name, save_directory):
	...
	# save_here = os.path.join(save_directory, library_name + '_signature.out')
	# #save_here = constantsModule.GENERAL_LIBRARY_TEMPLATE_FILE_PATH_NAME
	# with open(save_here, "w+") as fp:
	# 	fp.write('======\n')
	# 	fp.write("Library Name: %s\n\n"%library_name)
	# 	fp.write('======\n')
	# 	fp.write("Part 1: Sinks (Request Sending Functions)\n\n")
	# 	idx = 0
	# 	for chain in chain_functions:
	# 		if len(chain):
	# 			for target_function in chain:
	...
	# 					function_ids.append(target_function['id'])
	# 					fp.write('\t------\n\t%s) details:\n'%idx)
	# 					[signature, parameter, index] = _get_function_signature(target_function)	
	# 					fp.write("\t\tfunction_name: %s\n"%target_function['name'])
	# 					fp.write("\t\tfunction_signature: %s\n"%signature)
	# 					fp.write("\t\turl_parameter: %s\n"%parameter)
	# 					fp.write("\t\turl_parameter_index: %s\n"%index)
	# 					fp.write("\t\tfunction_definition_node_id: %s\n"%target_function['id'])
	# 					fp.write("\t\tfunction_definition_type: %s\n"%target_function['type'])
	# 					fp.write("\t\tfunction_definition_loc: %s\n"%target_function['loc'])
	...
	# 			fp.write("\t\tfunction_definition_type: %s\n"%function_def['Type'])
	# 			fp.write("\t\taffected `ThisExpression`: this.%s\n"%this_item_name)


	# 	fp.write("\n\n\nPart 2: Sources (Location Access)\n\n")
	# 	idx = 0
	# 	for chain in chain_functions_win_loc:
	# 		if len(chain):
	# 			for target_function in chain:
...
def run_analyzer(file_path, analyze=True, build=True,  activate= False, query=True, general=True):
	...
	build_wpg_for_library(file_path, analyze=analyze, build=build, activate=activate, general=general)
	if query:
