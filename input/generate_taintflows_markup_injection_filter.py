...
"""
  ...
  Description:
  ------------
  generates bash script that runs multiple instances of `scripts.filter_taintflows_by_injection_sinks` script 


  Usage:
  ------------
  $ python3 -m input.generate_taintflows_markup_injection_filter --parallel=20 --sitelist=$(pwd)/input/sitelist_final.csv
...
def main():
	...
	RUN_ALL_FILE_PATH = os.path.join(BASE_DIR, 'run_taintflow_markup_injection_filter.sh');
	STOP_ALL_FILE_PATH = os.path.join(BASE_DIR, 'stop_taintflow_markup_injection_filter.sh')
	...
	with open(RUN_ALL_FILE_PATH, 'w+') as fp1:
		with open(STOP_ALL_FILE_PATH, 'w+') as fp2:
			for i in range(parallel_instances):
				...
				cmd = "python3 -m scripts.filter_taintflows_by_injection_source_sinks --input={0} --from={1} --to={2}".format(sitelist_file_path_name, from_row, to_row)
