...
import wpg_neo4j.orm as ORMModule
...
class DOMUtils:
	...
	def resolve_dom_query_with_snapshot_obj(query, snapshot, bs4=False):
		...
		template_file = os.path.join(constantsModule.BASE_DIR, 'wpg_construction/lib/jaw/dom-points-to/template.example.js')
		...
		output_program = os.path.join(constantsModule.BASE_DIR, 'wpg_construction/lib/jaw/dom-points-to/template.js')
		# the results will be stored here by the node js program
		results_program = os.path.join(constantsModule.BASE_DIR, 'wpg_construction/lib/jaw/dom-points-to/resolution.out')
