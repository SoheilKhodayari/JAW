...
def build_and_analyze_hpg_local(seed_url, overwrite=False, conn_timeout=None, compress_hpg=True):
	...
	for webpage in webapp_pages:
		...
		if os.path.exists(webpage_folder):
			...
			try:
				DU.exec_fn_within_transaction(CSRFTraversalsModule.run_traversals, webpage_url, webpage_folder, webpage, conn=constantsModule.NEO4J_CONN_STRING, conn_timeout=conn_timeout)
