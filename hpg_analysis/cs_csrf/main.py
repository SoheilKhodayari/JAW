...
import re
import sys
import utils.selenium_utility as seleniumModule
import utils.dom_collector as DOMCollectorModule
import utils.html_parser as HTMLParserModule
...
def driver_program_web_page(site_identifier, navigation_url):
	site_identifier = str(site_identifier)
	site_id = site_identifier
	if site_identifier in sitesMapConfig.SITES_MAP:

		### set of variables for debug purposes
		override_existing_results= True # override template file if exists
		analyze = True # analyze the code to create a CSV graph
		build = True # build a property graph by importing the CSV
		query = True # query the property graph 
		use_existing_db_if_possible = False # override db if exists


		siteConfig = sitesMapConfig.SITES_MAP[site_id] 
		folder_name_of_url = siteConfig[0] + "_" + utilityModule._hash(navigation_url)
		...
		if not os.path.exists(absolute_program_folder_name):
			return -1
		else:
			template_file = os.path.join(absolute_program_folder_name, constantsModule.TEMPLATE_OUTPUT_SLUG)
			if not override_existing_results:
		...
		if analyze:
			[program_folder_name, absolute_program_folder_name] = neo4jDatabaseUtilityModule.API_build_property_graph(site_id, navigation_url)
		...
		if build:
			...
			if use_existing_db_if_possible and neo4jDatabaseUtilityModule.does_neo4j_db_exists(db_name):
...
def driver_program_web_site(site_identifier=None):

	if site_identifier is not None:
		site_identifier = str(site_identifier)
		site_id_list = [site_identifier] # test only a specific site
	else:
		site_id_list = sitesMapConfig.SITES_MAP.keys() # test all sites in site map

	for site_id in site_id_list:
		SITE_ID = site_id
		urls_directory = utilityModule.get_urls_directory(site_id)
		navigation_urls = utilityModule.get_urls_file_content(urls_directory)
...
if __name__ == "__main__":
	...
	if ACTIVE_MODE == ENUM_TEST_WEB_SITE:
		site_identifer = 1
		driver_program_web_site(site_identifier)
		...
		driver_program_unit_test(test_file_name='test_1.js')
