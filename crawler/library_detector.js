/**
 * [JS Dynamic Library Detector]
 *      @npm: https://www.npmjs.com/package/js-library-detector
 * 		reused from: https://github.com/johnmichel/Library-Detector-for-Chrome/blob/master/library/libraries.js
 * 		with minor modifications 
 * [Date: 12 Oct 2019]
 **/

var UNKNOWN_VERSION = null;
var JSLibraryDetectorTests = {
     ...
     */
     async function wait() {
        ...
        var detectLibraries = async function () {
            var tests = JSLibraryDetectorTests;
            var libraries = [];
            for (var i in tests) {
                try {
        ...
        if (window === top) {
            ...
            if (libs.length > 0) {
                let encodedLibs = encodeLibraries(libs);
                document.body.innerHTML += '<div class="lib_detection_id">' + encodedLibs + '</div>';
                //@Note Read libraries from DOM element with id lib_detection_id after, say, 10 seconds
                // console.log(encodedLibs)
            }
        }
    }
